...
public class StubDroidReader {
    private static final File STUB_DROID_DIR = new File(Config.getInstance().getStubDroidDir());
    private static final File STUB_DROID_MANUAL_DIR = new File(Config.getInstance().getStubDroidManualDir());
...
//                System.out.println("No summaries file for: " + className);
            }
        } catch (XMLStreamException | SummaryXMLException | IOException e) {
            log.error("Could not read summaries file: " + summariesFile.getAbsolutePath());
        }
        return null;
    }
}
